...
def kmean_anchors(path='./data/mydata.yaml', n=9, img_size=640, thr=4.0, gen=1000, verbose=True):
    """ Creates kmeans-evolved anchors from training dataset
        Arguments:
            path: path to dataset *.yaml, or a loaded dataset
            n: number of anchors
            img_size: image size used for training
            thr: anchor-label wh ratio threshold hyperparameter (siêu tham số) hyp['anchor_t'] used for training, default=4.0
    ...
    # Hàm xác định tỷ lệ giá trị weight và height dự đoán (wh) và giá trị weight và height đã cho trước (wh0)
    ...
    # xác định giá trị đột biến
    def fitness(k):  # mutation fitness
        _, best = metric(torch.tensor(k, dtype=torch.float32), wh)
        return (best * (best > thr).float()).mean()  # fitness

    # xác định tỷ lệ recall tức là tỉ lệ dự đoán đúng giá trị của wh trên tổng số wh0 và đưa ra nhận xét
    def print_results(k):
    ...
    if isinstance(path, str):  # *.yaml file
        ...
        from Yolov4_scaled.utils.datasets import LoadImagesAndLabels
    ...
    s = wh.std(0)  # sigmas for whitening (dùng standard diviation-độ lệch chuẩn để loại bỏ nhiễu trắng)
    ...
    # Evolve-Tiến hóa: lựa chọn các giá trị đột biến (mutate) tức là lựa chọn ngẫu nhiên v sao cho v*wh có giá trị gần bằng wh0
    npr = np.random
...
def check_anchors(dataset, model, thr=4.0, imgsz=640):
    ...
    # xác định giá trị của wh sau khi scale vượt quá ngưỡng  thì sẽ tính toán lại qua hàm kmean_anchors
...
def increment_dir(dir, comment=''):
    # Increments (gia tăng) a directory runs/exp1 --> runs/exp2_comment
    n = 0  # number
...
def compute_ap(recall, precision):
    ...
    # np.flip: đảo ngược thứ tự,
    mpre = np.flip(np.maximum.accumulate(np.flip(mpre)))

    # Integrate area under curve: tích phân tất cả các điểm dựa vào đồ thị của precision và recall
    method = 'interp'  # methods: 'continuous', 'interp'
    if method == 'interp':
        x = np.linspace(0, 1, 101)  # 101-point interp (COCO)
        # np.trapz: Trapezoidal rule: tích phân theo quy tắc hình thang để tính diện tích -> tổng diện tích ta được AP
        # np.interp: One-dimensional linear interpolation: quy tắc nội suy tuyến tính 1 chiều.
...
def ap_per_class(tp, conf, pred_cls, target_cls):
    # Tính dự đoán trung bình và đưa ra giá trị recall và percision curves
    """ Compute the average precision, given the recall and precision curves (đường cong).
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_gt == 0:
            ...
            # np.cumsum(axis=0): sum over rows for each of the columns
            fpc = (1 - tp[i]).cumsum(0)
            tpc = tp[i].cumsum(0)
    ...
    # Compute F1 score (harmonic mean of precision and recall): công thức so sánh độ tương quan giữa presion và recall
    # có giá trị từ 0->1, f1 càng tăng tức là độ chính xác nhận diện vật thể tăng.
    f1 = 2 * p * r / (p + r + 1e-16)
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False):
    ...
    if GIoU or DIoU or CIoU:
        ...
        if DIoU or CIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            c2 = cw ** 2 + ch ** 2 + 1e-16
            # centerpoint distance squared : khoảng cách tâm của
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(),
    ...
    if bucket:
        ...
        files = ['https://storage.googleapis.com/%s/res ults%g.txt' % (bucket, x) for x in id]
    else:
